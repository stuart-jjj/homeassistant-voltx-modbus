...
Address convention: doc_addr − 30001 = input frame addr; doc_addr − 40001 = holding frame addr.
  ...
  Holding registers (function code 0x03):
    1103        work_mode    Work mode                    u16  enum    2=Self-consumption,3=Reserve Power,4=Custom,5=ToU
    1150        cloud_status Commbox cloud comm status    u16  ×1      enum (0x000A=OK) — READ ONLY
    1151        chflg        Charge/discharge flag        u16  enum    1=Stop,2=Charging,3=Discharging — READ ONLY status
    1152        chpwr        Charge/discharge power cmd   s16  ×1      W (neg=charge, pos=discharge) — Custom mode control
...
class VoltxModbusCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def _fetch_data(self) -> dict[str, Any]:
        ...
        try:
            if not client.open():
                raise UpdateFailed(
                    f"Cannot open Modbus TCP connection to {self.host}:{self.port} (slave {self.slave_id})"
                )
            ...
            if regs is not None and len(regs) >= 75:
                ...
                data["tmp"] = (  # inverter temp °C
                    None
                    if tmp_raw == -32768  # 0x8000 = not fitted
            ...
            if sregs is not None and len(sregs) >= 55:
                ...
                data["chflg"] = _CHFLG.get(
                    chflg_raw, f"Unknown({chflg_raw})"
                )  # charge/discharge flag (read-only status)
    ...
    # Number of times to attempt a register write before giving up.
    _WRITE_ATTEMPTS = 3
    # Seconds to wait between write attempts.
    _WRITE_RETRY_DELAY = 1.0

    # Number of times to attempt a full data read before marking entities unavailable.
    _READ_ATTEMPTS = 3
    # Seconds to wait between read attempts (async sleep, does not block event loop).
    _READ_RETRY_DELAY = 2.0

    def _write_register(self, address: int, value: int) -> None:
        """Synchronous FC06 write with retry – called via async_add_executor_job.

        Retries up to _WRITE_ATTEMPTS times with a short delay between each
        attempt so transient TCP glitches do not immediately surface as errors
        in automations.
        """
        from pyModbusTCP.client import ModbusClient  # noqa: PLC0415
        ...
        for attempt in range(1, self._WRITE_ATTEMPTS + 1):
            ...
            try:
                if not client.open():
                    last_error = f"Cannot open Modbus TCP connection to {self.host}:{self.port} (slave {self.slave_id})"
                    _LOGGER.warning(
                        "Write attempt %d/%d – %s",
                        attempt,
                    ...
                    if client.write_single_register(address, value & 0xFFFF):
                        return  # success – exit immediately
                    last_error = f"Write to register {address} failed on {self.host}:{self.port} slave {self.slave_id}"
                    _LOGGER.warning(
                        "Write attempt %d/%d – %s",
                        attempt,
                        self._WRITE_ATTEMPTS,
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        Retries up to _READ_ATTEMPTS times with an async delay between each
        attempt so transient TCP hiccups do not immediately cause entities to
        flip to Unavailable in the UI.
        """
        ...
        for attempt in range(1, self._READ_ATTEMPTS + 1):
            try:
                ...
                last_exc = exc
                _LOGGER.warning(
                    "Read attempt %d/%d failed for %s:%d slave %d: %s",
                    attempt,
                    self._READ_ATTEMPTS,
                    self.host,
                    self.port,
                ...
                raise UpdateFailed(
                    f"Unexpected error communicating with {self.host}:{self.port} slave {self.slave_id}: {exc}"
                ) from exc
    ...
    async def async_validate_connection(self) -> None:
        ...
        try:
            ...
            raise ConfigEntryNotReady(
                f"Cannot connect to inverter at {self.host}:{self.port} slave {self.slave_id}: {exc}"
            ) from exc
        if not data:
            raise ConfigEntryNotReady(
                f"Inverter at {self.host}:{self.port} slave {self.slave_id} returned no data – check slave ID."
            )
